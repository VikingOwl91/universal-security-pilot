---
name: sec-fix
description: Remediate findings from a security audit report under the Wave Protocol with the Iron Law (no fix without a failing PoC test). Resists deadline and authority pressure via the rationalization counters. Invoke as `/sec-fix [audit-report-path]` or via natural language.
user-invocable: true
---
