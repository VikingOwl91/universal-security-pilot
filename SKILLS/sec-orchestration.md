...
## W5 ↔ W6 Dependency Handshake

Every W6 admission control has a corresponding W5 build-time prerequisite. Without the W5 fix, the W6 control either has no input to verify, or constrains a much wider blast radius than the policy assumes. Without the W6 enforcement, the W5 discipline is decorative — the artifact is hardened but the runtime admits any artifact regardless. **Both layers are required; neither alone is sufficient.**

| W5 prerequisite (build-time) | W6 enforcement (admission-time) | Failure mode when only one is fixed |
|---|---|---|
| **W5-4** Non-root `USER` directive in image (numeric UID) | **W6-4** PodSecurity `restricted` profile enforcing `runAsNonRoot: true` and `runAsUser: <uid>` | *W5 only:* image declares non-root, but a manifest without `securityContext.runAsNonRoot` still admits and (depending on entrypoint) may run as root. *W6 only:* admission rejects root pods, but any image granted an exception still has the larger root-shaped attack surface internally — and exception drift is the typical compromise vector. |
| **W5-7** `FROM <base>@sha256:<digest>` digest-pinned in Dockerfile | **W6-11** Admission rejects pods whose `image:` is not digest-pinned | *W5 only:* the Dockerfile is pinned, but the workload manifest can still reference `:latest` and the cluster pulls whatever the registry currently serves. *W6 only:* admission requires digests at the manifest layer, but every rebuild silently drifts the *base* image under the same Dockerfile tag. |
| **W5-9** Cosign signature emitted on every published image | **W6-10** Admission verifies signature against pinned OIDC identity, in `enforce` mode | *W5 only:* signing is decorative — the cluster admits images regardless of signature presence. *W6 only:* the policy has nothing to verify; either every image fails admission, or the policy is downgraded to `audit`/`warn` and falls into the W6-10 silent-killer pattern. |

A finding in either column with the corresponding other-column entry unfixed is a **higher-severity finding than either alone** — the defect compounds. Audit reports flag the cross-wave dependency explicitly: a W6-10 finding (no signature verification at admission) on a workload whose images already lack W5-9 signatures is a *paired* finding documented in both wave reports, not split between them.

The handshake table is incomplete by design. Some W5 rows (W5-2 `.dockerignore`, W5-12 layer-extraction) have no corresponding W6 admission control because the relevant defect cannot be detected at admission time without inspecting the image's *layers* (which admission controllers do not do today). Those W5 rows depend on W5-side discipline alone, and the audit notes the asymmetry rather than inventing a W6 row that doesn't exist.
...
#### W6-10 — Admission policy does not verify cosign signature on pulled images (or runs in `warn`/`audit` mode)
**Severity:** Critical  *(audit-mode admission is load-bearing false sense of security; the silent-killer pattern at W6)*
**Maps to:** OWASP A05 · A08 · ASVS V10.3.2 · V14.2.1 · CWE-345 · CWE-353 · CIS K8s 5.5.1 · NSA-CISA §IV
**Symptom in code:** Cluster has no `sigstore policy-controller` ClusterImagePolicy, no Kyverno `verifyImages` rule, no Connaisseur configuration — or one of the above is present but in `audit` / `warn` / `validationFailureAction: audit` mode rather than `enforce`; or the policy excludes the namespaces that actually run production workloads.
**Why it's wrong:** This is the W5↔W6 handshake on the provenance axis, and the **silent-killer instance at W6** — analogous to W0-13 (staged-diff-only scanner) and W5-12 (COPY-then-rm). W5-9 says "every published image is signed with cosign." W6-10 says "the cluster only admits images whose signature verifies against the expected identity." Without the admission verification, the publishing-side signing is decoration: any unsigned image (or an image signed by an unexpected identity) is admitted just the same. The OIDC-identity check is critical: a policy that verifies "any cosign signature exists" is not meaningfully stronger than no policy at all, since an attacker who can push to the registry can also sign with their own keyless identity. The policy must verify the signature against a specific Fulcio-issued certificate identity (`certificateIdentity` regex matching the team's CI OIDC subject) and a specific OIDC issuer. The `audit`/`warn` variant is the cruelest case — the policy *exists*, the dashboard *appears* clean, the warnings *are* emitted somewhere, but no failing assertion ever blocks an admission.

**Real-world scenario (the audit-mode theatre):**

A platform team at a mid-sized SaaS company finishes their cosign rollout. Every CI pipeline now signs images with sigstore keyless signing; every published image has a verifiable signature against the team's GitHub Actions OIDC identity. They install `sigstore policy-controller` to the cluster and configure a `ClusterImagePolicy` requiring signature verification. To minimize disruption during rollout, they set the policy mode to `warn` — pods that fail verification are admitted, but the policy emits a warning event. The plan: monitor warnings for two weeks, then flip to `enforce`.

The platform team posts the policy YAML to Slack with a thumbs-up. Security signs off. The dashboard shows zero warnings on day one — every production image is signed, every signature verifies. Two weeks pass. Three weeks pass. Six weeks pass. The dashboard still shows zero warnings of interest. The flip-to-enforce ticket gets deprioritized — the rollout already worked, why introduce risk now?

What nobody noticed: a stale `imagePullPolicy: IfNotPresent` deployment from a third-party Helm chart references `bitnami/redis:6.2-debian-10` (mutable tag, no digest pin — a separate W6-11 finding). The Helm chart was installed 18 months ago, before the cosign discipline existed. The image is unsigned. The admission policy emits a warning every time a Redis pod restarts. *That warning has been emitted 4,800 times over six weeks.* Nobody is reading it — Kubernetes events have a 1-hour default retention, the logging pipeline aggregates them under a noisy `PolicyWarning` bucket, and the on-call rotation muted that bucket two months ago because of unrelated cert-rotation noise. The warnings are technically observable; the warning channel is not.

A registry-side compromise then publishes a malicious image at `bitnami/redis:6.2-debian-10` — the attacker doesn't need to compromise Bitnami, just any mirror or pull-through proxy the cluster's nodes are configured to use. Next time the Redis pod restarts (Friday evening, K8s eviction), the malicious image is pulled. The admission policy emits a warning. The pod admits. The malicious Redis container has the namespace's full ServiceAccount token, can reach every other namespace via the cluster pod network (W6-8 not enforced for the legacy namespace either), and exfiltrates a backup of every `session:*` key over DNS to an attacker-controlled NS server before anyone notices the warning event.

Eight days later, customer accounts begin showing unauthorized access. The post-mortem reads: *"Cosign signature verification was configured. The policy was in `warn` mode, not `enforce`. Warning events emitted from the unsigned legacy chart had been routed to a muted alert bucket. No human read the policy-warning stream in over a month. The cluster admitted an unsigned malicious image because the gate, despite being installed, was advisory."*

What the Pilot would have produced instead: with `~/.security-pilot/SKILLS/sec-orchestration.md` in the agent's reasoning loop, the agent would have read W6-10's explicit rule — *"verifying signatures without enforcement is decoration"* — and refused to land the rollout in `warn` mode without a documented sunset date *and* a CI assertion that flips the policy to `enforce` on that date *and* a one-time inventory job that lists every existing-but-unsigned image so the legacy Helm chart's `bitnami/redis:6.2-debian-10` reference is surfaced and remediated *before* the policy goes hot. The W0-13 / W5-12 / W6-10 trilogy is the same shape three layers deep: a control that *appears* installed but, by configuration, fails to actually enforce. Audit-mode admission is exactly the W6 instance.
...
4. **Severity calibration review.** ✓ Assigned in v3.1-alpha. Distribution: **4 Critical** (W6-1 cluster-admin SA, W6-2 wildcard RBAC, W6-6 plaintext secret in YAML, W6-10 audit-mode admission — the silent-killer trilogy completion); **3 High** (W6-4 PodSecurity gap, W6-8 ingress default-deny missing, W6-9 egress default-deny missing); **4 Medium** (W6-3 default SA / automount, W6-5 writable root fs, W6-7 secret-as-env, W6-11 mutable-tag admission). W6-10 was promoted from High to Critical when the audit-mode scenario was added — the load-bearing-false-security pattern matches the W0-13 / W5-12 framing exactly. Re-review at non-alpha cut against any post-validation findings.
